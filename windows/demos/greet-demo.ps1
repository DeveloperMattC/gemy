...
$deploy = if ($NoPush) { "no push (board code left alone)" } else { "push + cleanup" }
Write-Host "  ($deploy + Moonshine + $mode)" -ForegroundColor DarkGray
Write-Host ""
...
if ($NoPush) {
    Write-Step "Skipping adb push (-NoPush) — using greeter already on the board..."
    $hasGreeter = (& adb shell "test -f /home/root/greeter.py && echo yes || echo no" 2>$null | Out-String).Trim()
    if ($hasGreeter -notmatch "yes") {
        Write-Host "ERROR: /home/root/greeter.py missing on board; cannot start without push." -ForegroundColor Red
        Write-Host "  Omit -NoPush only if you intend to overwrite the board with this repo." -ForegroundColor Yellow
        exit 1
    }
} else {
