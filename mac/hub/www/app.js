(function () {
  ...
  async function refreshAll() {
    setBusy(true, "Checking board (no push)…");
    try {
      ...
      showToast(
        data.message ||
          (data.health?.boardConnected ? "Board status updated" : "Refresh done — board offline"),
        data.ok ? "ok" : "err"
      );
      await loadBoardLog();
      await loadProcesses();
  ...
  $("btn-hat").addEventListener("click", () =>
    postAction("/api/hat-panel", null, "HAT beep test…", "HAT beep sent")
  );
  ...
  async function init() {
    try {
      ...
      showToast("Cannot reach Control Center server. Is the hub terminal still open?", "err");
